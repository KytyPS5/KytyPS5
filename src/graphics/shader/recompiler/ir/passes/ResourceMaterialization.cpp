...
bool MaterializeIndirectImage(const DescriptorSource::IndirectImage& indirect,
	...
	for (uint64_t offset = residue; offset <= limit && probe_count != 0u; offset += step) {
		uint32_t key = 0;
		if (!ReadScalarBufferWord(material, static_cast<uint32_t>(offset), 0u,
		                          runtime, key)) {
			if (offset > residue) {
				// Reached unmapped tail of buffer allocation; stop probing further words.
				break;
			}
			return false;
		}
	...
	next.capacity = static_cast<uint32_t>(std::max<size_t>(probe_count + 1u, keys.size()));
	...
	for (const auto key: next.keys) {
		...
		if (!candidate_valid || NullImageDescriptor(candidate) || !ValidImageDescriptor(candidate)) {
...
bool MaterializeDirectIndirectImage(const DescriptorSource::IndirectImage& indirect,
                                    ...
                                    ResourceSnapshot::IndirectImage& result) {
	uint64_t base = 0;
	if (heap_value.dword_count == 2) {
		base = static_cast<uint64_t>(heap_value.dwords[0]) |
		       (static_cast<uint64_t>(heap_value.dwords[1]) << 32u);
	} else if (heap_value.dword_count == 4) {
		ShaderBufferResource buffer;
		if (DecodeBufferDescriptor(heap_value, buffer)) {
			base = buffer.Base48();
		}
	}
	if (base == 0) {
	...
	const auto table_addr = base + indirect.selector_offset;
	const auto stride     = indirect.selector_stride != 0u ? indirect.selector_stride : 32u;

	std::vector<uint32_t>        keys;
	std::vector<DescriptorValue> descriptors;
	std::vector<uint32_t>        candidates;

	for (uint32_t key = 0; key < 16u; key++) {
		DescriptorValue candidate;
		candidate.dword_count = 8u;
		bool valid = true;
		for (uint32_t dword = 0; dword < 8u; dword++) {
			if (!ReadSpecializationWord(runtime, table_addr + key * stride + dword * sizeof(uint32_t),
			                            candidate.dwords[dword])) {
				valid = false;
				break;
			}
		...
		const bool is_null = NullImageDescriptor(candidate);
		const bool is_valid = ValidImageDescriptor(candidate);
		if (is_null || !is_valid) {
			if (key == 0u) {
				candidate.dwords.fill(0);
			} else {
		...
		keys.push_back(key);
		const auto found = std::ranges::find(descriptors, candidate);
		if (found == descriptors.end()) {
			descriptors.push_back(candidate);
			candidates.push_back(static_cast<uint32_t>(descriptors.size() - 1u));
		} else {
			candidates.push_back(static_cast<uint32_t>(found - descriptors.begin()));
		}
	}
	...
	result.resource    = 0;
	result.capacity    = static_cast<uint32_t>(keys.size());
...
bool ValidateResourceSpecialization(const Program& program, const ResourceSnapshot& snapshot) {
	...
	for (uint32_t i = 0; i < program.info.images.size(); i++) {
		...
		if (image.indirect_root != ImageResource::NoIndirectImage && image.indirect_root != i) {
			continue;
		}
		...
		if (NullImageDescriptor(descriptor)) {
			if (image.indirect_root != ImageResource::NoIndirectImage) {
				...
				if (root >= program.info.images.size() ||
				    program.info.images[root].resource_class != image.resource_class ||
				    program.info.images[root].numeric_class != image.numeric_class ||
			...
			const auto canonical_numeric = image.atomic ? Prospero::TextureNumericClass::Uint
			                                            : Prospero::TextureNumericClass::Float;
			if (image.dimension != Decoder::ImageDimension::Dim2D || image.cube ||
			    image.resource_class == ImageResourceClass::None ||
			    image.numeric_class != canonical_numeric) {
				return false;
			}
			continue;
		}
		...
		if (image.resource_class == ImageResourceClass::None) {
			return false;
		}
		const bool storage = image.resource_class == ImageResourceClass::Storage;
		const auto format =
		...
		auto numeric_class = Prospero::SampledTextureNumericClass(format);
		if (storage) {
			const bool raw_sint = format == Prospero::BufferFormat::k32SInt && image.written &&
			                      !image.read && !image.atomic;
			if ((!raw_sint && numeric_class == Prospero::TextureNumericClass::Sint) ||
			    numeric_class == Prospero::TextureNumericClass::Unsupported) {
				return false;
			}
			if (raw_sint) {
				numeric_class = Prospero::TextureNumericClass::Uint;
			}
		} else if (image.depth_compare && numeric_class != Prospero::TextureNumericClass::Float) {
			return false;
		}
		if (image.numeric_class != numeric_class) {
			return false;
		}
	}
	return true;
...
bool MaterializeResources(const Program& program, const SrtRuntime& runtime,
	...
	for (const auto& image: program.info.images) {
		...
		if (source != nullptr && source->indirect_image.has_value()) {
			if (runtime.read_specialization_memory == nullptr) {
				if (runtime.userdata == nullptr) {
					continue;
				}
				return false;
			}
			if (source->indirect_image->material_source != UINT32_MAX) {
	...
	for (uint32_t image_index = 0; image_index < program.info.images.size(); image_index++) {
		...
		if (source != nullptr && source->indirect_image.has_value()) {
			...
			if (runtime.read_specialization_memory == nullptr && runtime.userdata == nullptr) {
				table.capacity = 1u;
				table.keys     = {0u};
				DescriptorValue null_desc;
				null_desc.dword_count = 8u;
				null_desc.dwords.fill(0);
				table.descriptors = {null_desc};
				table.candidates  = {0u};
			} else if (source->indirect_image->material_source == UINT32_MAX) {
				...
				const auto& heap = tables[0];
				if (!MaterializeDirectIndirectImage(*source->indirect_image, heap, runtime, table)) {
					if (image.indirect_root == ImageResource::NoIndirectImage && runtime.userdata == nullptr) {
						table.capacity = 1u;
						table.keys     = {0u};
						DescriptorValue null_desc;
						null_desc.dword_count = 8u;
...
void SpecializeResources(Program& program, ResourceSnapshot& snapshot) {
	...
	for (uint32_t i = 0; i < next.images.size(); i++) {
		...
		if (image.resource_class == ImageResourceClass::None ||
		    (image.atomic && image.resource_class != ImageResourceClass::Storage)) {
		...
		if (NullImageDescriptor(descriptor)) {
			image.numeric_class = image.atomic ? Prospero::TextureNumericClass::Uint
			                                   : Prospero::TextureNumericClass::Float;
	...
	for (uint32_t root_index = 0; root_index < next.images.size(); root_index++) {
		...
		for (uint32_t candidate = 0; candidate < next.images.size(); candidate++) {
			...
			if (NullImageDescriptor(next_snapshot.images[candidate])) {
				image.resource_class    = image_class.resource_class;
			...
			if (image.resource_class != image_class.resource_class ||
			    image.numeric_class != image_class.numeric_class ||
			    image.dimension != image_class.dimension ||
			    ...
			    image.cube != image_class.cube) {
				std::printf("warning: indirect image table at pc 0x%08x harmonizing candidate %u (num=%u conv=%u) to exemplar %u (num=%u conv=%u)\n",
				            root.first_use_pc, candidate,
				            static_cast<uint32_t>(image.numeric_class),
				            static_cast<uint32_t>(image.conversion_format),
				            exemplar,
	...
	for (const auto& pair: next.sampled_pairs) {
		...
		if (RequiresPointSampler(next.images[pair.image])) {
