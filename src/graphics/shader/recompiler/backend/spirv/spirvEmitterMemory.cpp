...
uint32_t GuestAddress(ValueEmitContext& ctx, const IR::Inst& inst, const IR::MemoryInfo& mem) {
	...
	if (mem.address_is_full) {
		...
		auto base_high = ctx.Arg(*handle, 1);
		if (mem.kind == IR::ResourceKind::ScalarAddress) {
			// Guest addresses are 48 bits wide: the host scalar path masks the combined address
			// (SrtWalker's AddressMask), so the device address must drop the upper bits of the high
			// word as well.
			base_low =
			    Binary(state, spv::OpBitwiseAnd, TypeU32(state), base_low, ConstantU32(state, ~3u));
			base_high = Binary(state, spv::OpBitwiseAnd, TypeU32(state), base_high,
			                   ConstantU32(state, 0xffffu));
		}
		const auto base = DeviceAddressFromWords(state, base_low, base_high);
