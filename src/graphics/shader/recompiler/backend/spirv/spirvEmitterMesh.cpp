...
uint32_t MeshElement(EmitterState& state, uint32_t variable, spv::StorageClass storage,
	...
	state.builder.AddFunction(
	    {spv::OpAccessChain, TypePointer(state, storage, type), pointer, variable, index});
	return pointer;
}
...
void EmitMeshEntryPoint(EmitterState& state) {
	...
	state.builder.AddFunction(
	    {spv::OpFunctionCall, TypeVoid(state), state.builder.AllocateId(), state.mesh_guest_func});
	...
	state.builder.AddFunction(
	    {spv::OpSetMeshOutputsEXT, vertices, primitives}); // OpSetMeshOutputsEXT
	for (uint32_t half = 0; half < state.lane_count; half++) {
		...
		state.builder.AddFunction(
		    {spv::OpULessThan, TypeBool(state), is_primitive, index, primitives});
		EmitIfCondition(state, is_primitive, [&] {
			...
			state.builder.AddFunction(
			    {spv::OpINotEqual, TypeBool(state), culled, null_bit, ConstantU32(state, 0)});
