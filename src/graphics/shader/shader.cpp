...
static void ShaderGetStaticInputInfoPS(
	...
	for (int i = 0; i < 8; i++) {
		...
		ps_info.target_channel_type[i]   = sh.target_channel_type[i];
		ps_info.target_format[i]         = sh.target_format[i];
...
void BuildStageStaticKey(const ShaderPixelInputInfo& info, std::vector<uint32_t>& key) {
	...
	key.insert(key.end(), std::begin(info.target_channel_type), std::end(info.target_channel_type));
	key.insert(key.end(), std::begin(info.target_format), std::end(info.target_format));
...
static void DumpShaderRecompilerSpirv(const char* type, uint64_t shader_hash,
	...
	const auto base_name = Config::GetShaderLogFolder() /
	                       fmt::format("{:04d}_new_shader_{}_{:016x}", current_id, type, shader_hash);
...
static int DumpShaderRecompilerInput(const char* type, uint64_t shader_hash,
...
static void DumpShaderRecompilerDisasm(const char* type, uint64_t shader_hash,
                                       int current_id,
...
static vk::ShaderModule CompileModule(vk::Device device, const ShaderParams& params,
	...
	int dump_id = DumpShaderRecompilerInput(stage_name, options.shader_hash, params.code);
	auto result = ShaderRecompiler::Recompile(params.code, options);

	DumpShaderRecompilerDisasm(stage_name, options.shader_hash, dump_id, result.decoded_dump);
	if (!SpirvValidateBinary(label, options.shader_hash, result.spirv)) {
		DumpShaderRecompilerSpirv(stage_name, options.shader_hash, dump_id, result.spirv);
