...
uint32_t EmitSdwaDestinationMerge(EmitterState& state, uint32_t pointer, const IR::Operand& dst,
	...
	if (dst.sdwa_dst_unused == 0u) {
		return shifted;
	}
	if (dst.sdwa_dst_unused == 1u) {
		const auto extended = state.builder.AllocateId();
		const auto result   = offset != 0u ? state.builder.AllocateId() : extended;
		state.builder.AddFunction({OpBitFieldSExtract, state.uint_type, extended, value,
		                           ConstantU32(state, 0), ConstantU32(state, width)});
		if (offset != 0u) {
			state.builder.AddFunction({OpShiftLeftLogical, state.uint_type, result, extended,
			                           ConstantU32(state, offset)});
		}
		return result;
	}
	EXIT_IF(dst.sdwa_dst_unused != 2u);
