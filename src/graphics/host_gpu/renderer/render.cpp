...
template <typename T>
static T DecodeNativeDescriptor(const ShaderRecompiler::IR::DescriptorValue& value) {
	T result {};
...
static PipelineDynamicParameters BuildGraphicsDynamicParams(const HW::Context&     ctx,
	...
	ret.vk_dynamic_state_line_width             = true;
	ret.vk_dynamic_state_viewport               = true;
	ret.vk_dynamic_state_scissor                = true;
	ret.vk_dynamic_state_stencil_compare_mask   = true;
	ret.vk_dynamic_state_stencil_reference      = true;
	ret.vk_dynamic_state_stencil_write_mask     = true;
	ret.vk_dynamic_state_color_write_enable_ext = true;
...
static bool MarkShaderAddressWrites(const std::vector<ShaderAddressWriteRange>& writes) {
	if (!writes.empty()) {
...
static bool HasShaderBufferWrites(const ShaderStageRuntime& runtime) {
	EXIT_IF(!runtime);
	return !CollectShaderBufferWrites(*runtime.program, *runtime.resources).empty();
}

static void ShaderWriteBarrier(VkCommandBuffer vk_buffer, VkPipelineStageFlags source_stages) {
