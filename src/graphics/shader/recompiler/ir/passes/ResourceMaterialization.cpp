	...
	for (const auto& read: program.physical_address_reads) {
		...
		for (const auto offset: offsets) {
			...
			if (address > 0 && static_cast<uint64_t>(address) <= AddressMask - 3u) {
				next.physical_read_ranges.push_back({static_cast<uint64_t>(address), 4});
			}
		}
	}
	std::ranges::sort(next.physical_read_ranges, {}, &ResourceSnapshot::AddressRange::address);
	size_t range_count = 0;
	for (const auto range: next.physical_read_ranges) {
		if (range_count != 0) {
			auto& previous = next.physical_read_ranges[range_count - 1];
		...
		next.physical_read_ranges[range_count++] = range;
	}
	next.physical_read_ranges.resize(range_count);
