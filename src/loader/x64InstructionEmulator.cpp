...
// Bit-identical to glibc's four uint32_t '_xmm[i].element[4]' lanes on little-endian x86_64, so
// this can be reinterpret_cast onto them to get the same Low/High 64-bit view Win32's M128A gives.
struct Xmm128 {
	uint64_t Low;
	uint64_t High;
};

static Xmm128* GetContextXmm(ucontext_t* context, uint8_t index) {
	if (context == nullptr || context->uc_mcontext.fpregs == nullptr || index >= 16) {
		return nullptr;
	}

	return reinterpret_cast<Xmm128*>(&context->uc_mcontext.fpregs->_xmm[index]);
}
...
static bool TryEmulateSse4a(ucontext_t* context) {
	...
	if (prefix == 0x66) {
		...
		dst->Low  = ExtractBitField(dst->Low, length, index);
		dst->High = 0;
		context->uc_mcontext.gregs[REG_RIP] += static_cast<greg_t>(offset + 5);
		return true;
	}
