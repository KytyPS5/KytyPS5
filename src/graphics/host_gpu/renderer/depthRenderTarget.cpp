...
static vk::StencilOp ConvertStencilOp(uint8_t value, uint8_t write_mask, uint8_t op_value) {
	...
	switch (static_cast<Prospero::StencilOp>(value)) {
		...
		case Prospero::StencilOp::kReplaceOp:
			return (op_value & write_mask) == 0 ? vk::StencilOp::eZero : vk::StencilOp::eReplace;
...
static uint8_t ResolveStencilReference(uint8_t compare, uint8_t compare_mask, uint8_t write_mask,
                                       uint8_t test_value, uint8_t op_value, uint8_t fail,
                                       uint8_t pass, uint8_t depth_fail) {
	const auto comparison = static_cast<vk::CompareOp>(compare);
	const auto uses       = [&](Prospero::StencilOp operation) {
		const auto value = static_cast<uint8_t>(operation);
		return (comparison != vk::CompareOp::eAlways && fail == value) ||
		       (comparison != vk::CompareOp::eNever && (pass == value || depth_fail == value));
	};
	// Zero replacement has a native operation and consumes no reference bits.
	if ((op_value & write_mask) == 0 || !uses(Prospero::StencilOp::kReplaceOp)) return test_value;
	const uint8_t test_bits =
	    ((comparison == vk::CompareOp::eAlways || comparison == vk::CompareOp::eNever)
	         ? 0
	         : compare_mask) |
	    (uses(Prospero::StencilOp::kReplaceTest) ? write_mask : 0);
	if (((test_value ^ op_value) & test_bits & write_mask) != 0) {
		DepthFatal("incompatible stencil reference bits: compare=%u mask=%02x write=%02x test=%02x "
		           "op=%02x operations=%u,%u,%u",
		           compare, compare_mask, write_mask, test_value, op_value, fail, pass, depth_fail);
	}
	// Vulkan shares the compare and replacement reference. Preserve every bit
	// used by the test while inserting the bits required by the write mask.
	return (test_value & ~write_mask) | (op_value & write_mask);
}
...
void RenderExecutor::ResolveRenderDepthTarget(CommandBuffer& buffer, RenderDepthInfo& r) {
	...
	if (r.stencil_test_enable) {
		...
		if (dc.stencilfunc > static_cast<uint8_t>(vk::CompareOp::eAlways) ||
			...
			DepthFatal("unsupported stencil comparison: front=%u back=%u", dc.stencilfunc,
			           dc.stencilfunc_bf);
		}
		...
		r.stencil_dynamic_front = {sm.stencil_mask, front_write_mask,
		                           ResolveStencilReference(dc.stencilfunc, sm.stencil_mask,
		                                                   front_write_mask, sm.stencil_testval,
		                                                   sm.stencil_opval, sc.stencil_fail,
		                                                   sc.stencil_zpass, sc.stencil_zfail)};
		if (dc.backface_enable) {
			...
			r.stencil_dynamic_back = {
			    ...
			    ResolveStencilReference(dc.stencilfunc_bf, sm.stencil_mask_bf, back_write_mask,
			                            sm.stencil_testval_bf, sm.stencil_opval_bf,
			                            sc.stencil_fail_bf, sc.stencil_zpass_bf,
			                            sc.stencil_zfail_bf)};
		} else {
