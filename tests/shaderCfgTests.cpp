...
void TestNewShaderRecompilerExpPixelOutputs() {
  ...
  ShaderPixelInputInfo target_channel_uint_info;
  target_channel_uint_info.target_output_mode[0] = 1;
  target_channel_uint_info.target_channel_type[0] =
      static_cast<uint8_t>(Prospero::ChannelType::kUInt);
  target_channel_uint_info.target_format[0] =
      static_cast<uint8_t>(Prospero::ChannelLayout::k32);
  options.input_info.pixel = &target_channel_uint_info;
  auto target_channel_uint_result =
      ShaderRecompiler::Recompile(partial_shader, options);
  const auto target_channel_uint_source =
      DisassembleSpirvBinary(target_channel_uint_result.spirv);
  Check(Common::ContainsStr(target_channel_uint_source,
                            "OpVariable %_ptr_Output_v4uint Output"),
        "Target channel kUInt did not use an unsigned integer output variable");
  Check(!Common::ContainsStr(target_channel_uint_source, "UnpackUnorm4x8"),
        "Target channel kUInt incorrectly unpacked UNORM 4x8");
  CheckSpirvBinaryValidates(target_channel_uint_result.spirv);
