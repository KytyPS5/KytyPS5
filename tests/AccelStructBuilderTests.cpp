...
void TestBuildsABlasFromASingleTriangle() {
	...
	Check(bool(resources.handle),
	      "acceleration structure handle was null after a successful build");
	...
	const auto as_address =
	    VULKAN_HPP_DEFAULT_DISPATCHER.vkGetAccelerationStructureDeviceAddressKHR(
	        device,
	        static_cast<const VkAccelerationStructureDeviceAddressInfoKHR*>(as_address_info));
