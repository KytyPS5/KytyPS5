...
static void PatchProgram(Program* program, uint64_t address, uint64_t size) {
	...
	if (!program->elf->IsShared() && program->tls.handler_vaddr != 0 &&
		...
		for (auto* ptr = start_ptr; ptr < end_ptr;) {
			...
			const uint8_t modrm = remaining >= inst_size ? inst_ptr[3] : 0xff;
			if (instruction.length == inst_size && remaining >= inst_size &&
				...
				// Preserve the prefixes. Call9 starts with REX.W to neutralize 0x66
