...
static bool audioout2_host_audio_disabled() {
...
int KYTY_SYSV_ABI AudioOut2Initialize() {
	...
	// Diagnostic mode: take the normal unavailable-device path instead of creating a host audio
	// worker. This is intentionally opt-in for troubleshooting guest audio races.
	if (audioout2_host_audio_disabled()) {
		LOGF("AudioOut2: initialization disabled by KYTY_DISABLE_HOST_AUDIO\n");
