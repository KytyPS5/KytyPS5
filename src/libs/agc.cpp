...
// neither the driver pipeline cache nor the shader cache would ever reach disk
// if they were written from a destructor. std::at_quick_exit is the hook that
// does run, and at that point the device and both caches are still alive.
// (The driver pipeline cache also has a save on WindowRun()'s normal-exit path
// via GetPipelineCache().Save(); this is the non-graceful-exit backstop for it
// too, via WindowFlushCaches().)
