...
bool IsAccessible(DWORD protect, HostMemoryAccess access) {
	...
	const DWORD wanted = (access == HostMemoryAccess::Write ? writable : readable);
	return (protect & blocked) == 0 && (protect & wanted) != 0;
}
#endif
...
bool HostMemoryQueryRange(uint64_t addr, uint64_t requested_size, HostMemoryAccess access,
	...
	while (current < end && std::fgets(line, sizeof(line), maps) != nullptr) {
		...
		const bool allowed =
		    access == HostMemoryAccess::Mapped ||
		    (permissions[0] == 'r' && (access != HostMemoryAccess::Write || permissions[1] == 'w'));
		if (!allowed) {
			break;
		}
		current = finish < end ? finish : end;
...
bool HostMemoryRangeIsWritable(uint64_t addr, uint64_t size) {
	...
	return HostMemoryQueryWritable(addr, size, writable_size) && writable_size >= size;
}
