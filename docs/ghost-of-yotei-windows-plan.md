...
Сейчас быстрый цикл вернул текущую сборку к frame 18 после устранения runtime
регрессии на `5be616…`: простой acyclic WorkgroupId coefficient read снова
исполняется через BDA на GPU, а 16 snapshot-чтений циклического `916e…`
сохранены. Следующие практические цели — уменьшить стоимость `e52e…`/`916e…`
и найти первую неверную запись в source image, которая остаётся чёрной при
успешных flips.
...
| --- | --- | --- |
| Установленный эмулятор | Текущая Windows-сборка `c6307e1-dirty`, SHA-256 `64f2777bc6ce666399c6a60ffae718de54dbcdf52579e2d89ddd4e4889694c96`; build и install завершились успешно. | Dirty соответствует ещё не закоммиченному acyclic WorkgroupId исправлению; после коммита banner/hash изменятся. |
...
| CPU-аудит корпуса | `yotei-fast-cycle-20260907-01`: **825 manifests, 714 passed / 111 failed** по текущей строгой границе; это +28 к последнему сопоставимому batch. | `passed` означает достигнутую стадию статического аудита, а не готовность к GPU. |
| Реальная игра | `220148-096a03`: frame 18, FPS 0,068648, 5 GPU flips, 4 shown; graceful timeout, exit 0. Предыдущий half-resolution readback подтвердил полностью нулевой RGB. | Первый ненулевой видимый кадр ещё не достигнут. |
...
| Каталог последнего запуска | `_Build/runs/yotei-integrated-20260906-220148-096a03` |
| SHA-256 запущенного emulator | `64f2777bc6ce666399c6a60ffae718de54dbcdf52579e2d89ddd4e4889694c96` |
| Время UTC | `2026-09-06T22:01:48.7741313Z` → `22:03:19.3303282Z` |
...
| Наблюдаемое исполнение | frame 18; FPS 0,068648; flips CPU/GPU 0/5; prepared 5, ready 5, shown 4 |
...
| Точность состояния | `5be616…` больше не требует CPU snapshot грязной GPU-памяти; целевой audit сохранил 16 bounded reads у циклического `916e…`; исходная группа 7/7 остаётся без прежней resource validation ошибки |
